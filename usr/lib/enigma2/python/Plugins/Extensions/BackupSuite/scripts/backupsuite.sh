...
# Rilevamento versione OpenVision
...
# Usa opzioni standard per ls
LS_OPTIONS="-1rSh"
if ! ls $LS_OPTIONS / >/dev/null 2>&1; then
    LS_OPTIONS="-lSr"
fi

# Rilevamento Python migliorato
find_python() {
    # Cerca python3 prima, poi python2, poi python
...
# Definizione nome modulo Python
PYNAME="message"
...
# Fallback a .py se .pyc non esiste
...
# Impostazione dinamica di HARDDISK in base al tipo di dispositivo
if [ "$2" = "HDD" ]; then
    export HARDDISK=1
elif [ "$2" = "USB" ]; then
    export HARDDISK=0
...
# Calcolo dimensioni file (metodo portabile)
